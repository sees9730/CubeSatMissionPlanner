...
1. **Targets_Info**: Scientific targets for observation
2. **Survey_Info**: Survey configurations and priorities
3. **PlanPropagation_Info**: Basic mission parameters
4. **GroundStations_Info**: Ground station information
5. **Constraints_Info**: Operational constraints
6. **PowerBudget_Info**: Energy consumption rates
7. **DataBudget_Info**: Data generation rates
8. **SAA_Info**: South Atlantic Anomaly boundary coordinates
...
* **Target**: Target identifier/name (string)
* **Pointing**: Pointing ID for this target (integer or string)
    - For a target with multiple pointings, each pointing could be a number from 1 to N.
    - For a target with a single pointing, use something like "A". 
